...
TEST(CostFunctionTests, DenseMatrices) {
  ...
  EXPECT_EQ(cost.InputDimension(), func0->InputDimension());
  EXPECT_EQ(cost.InputDimension(), func1->InputDimension());
  EXPECT_EQ(cost.numPenaltyFunctions, 2);
  EXPECT_EQ(cost.numTerms, func0->OutputDimension()+func1->OutputDimension());
  ...
  EXPECT_EQ(jac.cols(), func0->InputDimension());
  EXPECT_EQ(jac.cols(), func1->InputDimension());
  EXPECT_EQ(jac.rows(), func0->OutputDimension()+func1->OutputDimension());
  EXPECT_NEAR((jac.topLeftCorner(func0->OutputDimension(), func0->InputDimension())-func0->Jacobian(beta)).norm(), 0.0, 1.0e-14);
  EXPECT_NEAR((jac.bottomLeftCorner(func1->OutputDimension(), func1->InputDimension())-func1->Jacobian(beta)).norm(), 0.0, 1.0e-14);

  const Eigen::VectorXd grad = cost.Gradient(eval, jac);
  EXPECT_EQ(grad.size(), func0->InputDimension());  
  EXPECT_EQ(grad.size(), func1->InputDimension());  
  EXPECT_NEAR((grad-cost.Gradient(beta)).norm(), 0.0, 1.0e-14);
  Eigen::VectorXd gradFD = Eigen::VectorXd::Zero(func0->InputDimension());
  { // estimate the gradient with 8th order finite differences
    ...
    for( std::size_t component=0; component<func0->InputDimension(); ++component ) {
  ...
  Eigen::MatrixXd hessFD = Eigen::MatrixXd::Zero(func0->InputDimension(), func0->InputDimension());
