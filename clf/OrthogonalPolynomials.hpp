...
/// Evaluate orthogonal polynomial basis functions \f$\phi_p: \mathcal{D} \mapsto \mathbb{R}\f$ of order \f$p\f$.
/**
We use the recursive relationship to evaluate the \f$p^{\text{th}}\f$ degree polynomial
\f{eqnarray*}{
   \phi_0(x) &=& \phi_0 \\
   \phi_1(x) &=& \phi_1(x) \\
   \phi_p(x) &=& (A_p x + B_p) \phi_{p-1}(x) - C_p \phi_{p-2}(x) \quad \text{for} \quad p>1.
\f}
The \f$k^{\text{th}}\f$ derivative is
\f{eqnarray*}{
   \frac{d^{k}}{d x^{k}}\phi_0(x) &=& 0 \quad \text{for all} \quad k>0 \\
   \frac{d^{k}}{d x^{k}}\phi_1(x) &=& \phi_1 \quad \text{for} \quad k=1 \quad \text{and} \quad \frac{d^{k}}{d x^{k}}\phi_1(x) = 0 \quad \text{for all} \quad k>1 \\
   \frac{d^{k}}{d x^{k}}\phi_p(x) &=& 0 \quad \text{for} \quad p>1,\, k > p \quad \text{and} \quad \frac{d^{k}}{d x^{k}}\phi_p(x) = k A_p \frac{d^{k-1}}{d x^{k-1}}\phi_{p-1}(x) + (A_p x + B_p) \frac{d^{k}}{d x^{k}}\phi_{p-1}(x) - C_p \frac{d^{k}}{d x^{k}}\phi_{p-2}(x) \quad \text{for} \quad p>1,\, 0 < k \leq p,
\f}
using the notation that \f$\frac{d^0}{dx^0} \phi_p(x) = \phi_p(x)\f$.
*/
...
public:
  ...
  */
  ...
  /**
     ...
     \return The vector of basis functions \f$[\phi_{0}(x),\, ...,\, \phi_{p}(x)]\f$.
  */
  virtual Eigen::VectorXd EvaluateAll(int const p, double const x) const override;

  /// Evaluate the basis function derivative \f$\frac{d^{k}}{dx^{k}} \phi_{p}(x)\f$.
  /**
     @param[in] p The order of the basis function 
     ...
     \return The basis function derivative \f$\frac{d^{k}}{dx^{k}} \phi_{p}(x)\f$.
  */
  ...
  /**
     ...
     \f{equation*}{
     D = \begin{bmatrix}
     \frac{d}{dx} \phi_0(x) & ... & \frac{d^k}{dx^k} \phi_0(x) \\
     \vdots & \ddots & \vdots \\
     \frac{d}{dx} \phi_p(x) & ... & \frac{d^k}{dx^k} \phi_p(x) \\
     \end{bmatrix}.
     \f}
     Override this function so we do not unnecessarily keep calling the recursive evaluate function.
     @param[in] p Evaluate up to the \f$p^{\text{th}}\f$ order of the basis function 
...
private:
  ...
  /**
     \return The first orthogonal polynomial \f$\phi_0\f$, which is constant with respect to \f$x\f$.
   */
  ...
  /**
     ...
     \return The second orthogonal polynomial \f$\phi_1\f$, which is linear with respect to \f$x\f$.
   */
