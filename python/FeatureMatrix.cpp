...
void clf::python::FeatureMatrixWrapper(py::module& mod) {
  ...
  mat.def(py::init<std::shared_ptr<const FeatureVector> const&, std::size_t const, std::shared_ptr<Domain> const&>());
  mat.def(py::init<std::vector<std::shared_ptr<const FeatureVector> > const&, std::shared_ptr<Domain> const&>());
  mat.def(py::init<std::vector<FeatureMatrix::VectorPair> const&, std::shared_ptr<Domain> const&>());
