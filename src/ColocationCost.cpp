...
SparseCostFunction(colocationCloud->OutputDimension()*colocationCloud->supportCloud->NumSupportPoints(), colocationCloud->OutputDimension()*colocationCloud->numColocationPoints),
...
void ColocationCost::ComputeOptimalCoefficients(Eigen::MatrixXd const& data) const {
  ...
  assert(data.cols()==colocationCloud->supportCloud->NumSupportPoints());

  for( auto point=colocationCloud->supportCloud->Begin(); point!=colocationCloud->supportCloud->End(); ++point ) {
...
Eigen::VectorXd ColocationCost::CostImpl(Eigen::VectorXd const& data) const {
  // compute the optimal coefficients for each support point
  return ComputeCost(Eigen::Map<const Eigen::MatrixXd>(data.data(), colocationCloud->OutputDimension(), colocationCloud->supportCloud->NumSupportPoints()));
}

void ColocationCost::JacobianImpl(Eigen::VectorXd const& data, Eigen::SparseMatrix<double>& jac) const {
  Eigen::Map<const Eigen::MatrixXd> dataMat(data.data(), colocationCloud->OutputDimension(), colocationCloud->supportCloud->NumSupportPoints());
